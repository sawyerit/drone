...
namespace Drone.API.Dig.Dns
{
	...
	public class Resolver : IDisposable
	{
		...
		public Resolver(IPEndPoint[] DnsServers)
		{
			...
			_cacheThread = new Thread(CleanUpCache);
			_cacheThread.Start();
		}
		...
		private Response SearchInCache(Question question)
		{
			...
			lock (m_ResponseCache)
			{
				...
				m_ResponseCache[strKey].TimeStamp = DateTime.Now;
		...
		private void CleanUpCache()
		{
			while (true)
			{
				Dictionary<string, Response> newCache = new Dictionary<string, Response>();

				lock (m_ResponseCache)
				{
					foreach (var item in m_ResponseCache)
					{
						if (DateTime.Now.Subtract(item.Value.TimeStamp) < TimeSpan.FromMinutes(30))
						{
