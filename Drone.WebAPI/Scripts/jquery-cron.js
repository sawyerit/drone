...
(function ($) {
    ...
    // options for period
    var str_opt_period = "";
    //    var periods = ["minute", "hour", "day", "week", "month", "year"]; ss: removed year option
    var periods = ["minute", "hour", "day", "week", "month"];
    ...
    var toDisplay = {
        ...
        "week": ["dow", "time"],
        "month": ["dom", "time"]//,
        //"year"   : ["dom", "month", "time"] ss: removed year option
    };
    ...
    var combinations = {
        ...
        "month": /^(\d{1,2}\s){3}\*\s\*$///,           // "? ? ? * *"
        //"year"   : /^(\d{1,2}\s){4}\*$/                // "? ? ? ? *" ss: removed year option
    };
    ...
    function getCurrentValue(c) {
        ...
        switch (selectedPeriod) {
            ...
            case "week":
                min = b["time"].find("select.cron-time-min").val();
                hour = b["time"].find("select.cron-time-hour").val();
                var dowVal = b["dow"].find("select").val();                
                dow = dowVal == "8" ? "MON-FRI" : dowVal == "9" ? "SAT,SUN" : dowVal;
                day = "?";
                break;
    ...
    var methods = {
        ...
        value: function (cron_str) {
            ...
            cron_str = convertFromQuartzStyle(cron_str);
    ...
    function convertFromQuartzStyle(cron_str){
        cron_str = cron_str.replace("MON-FRI", "8").replace("SAT,SUN", "9").replace("?", "*");
        if (cron_str.length > 10)
            cron_str = cron_str.substring(2, cron_str.length);
